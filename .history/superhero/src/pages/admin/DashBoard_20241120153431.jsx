...
const AdminDashboard = () => {
    ...
    // Handle status change and trigger email if status is 'Solved'
    const handleStatusChange = async (id, newStatus, email, name, problemDescription) => {
        try {
            // Update the grievance status first
            ...
            if (response.ok) {
                // If status is 'Solved', trigger email
                if (newStatus === "Solved") {
    ...
    const handleSendEmail = async (id, email, name, problemDescription) => {
        try {
            const response = await fetch(`http://localhost:5000/api/grievances/send-email`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ email, name, problemDescription }),
            });
                                            ...
                                            <select
                                                ...
                                                onChange={(e) =>
                                                    handleStatusChange(
                                                        row._id,
                                                        e.target.value,
                                                        row.email, // Pass email to send email after status change
                                                        row.name, // Pass name
                                                        row.problemDescription // Pass problem description
                                                    )
                                                }
                                            >
